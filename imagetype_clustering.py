...
STAGE1_TRAIN = "./data-science-bowl-2018/stage1_train"
...
X = (pd.DataFrame(trainPD[HSV_DOMINANT].values.tolist())).to_numpy()
...
kmeans.predict(np.array([[0.6397599475762402, 0.1359188139148213, 0.771]]))
...
def plot_images(images, images_rows, images_cols):
    ...
    for row in range(images_rows):
        for col in range(images_cols):
            ...
            ax = axarr[row,col]
            ax.axis('off')
            ax.set_title("%s - %dx%d"%(image_id, width, height))
            ax.imshow(image)
